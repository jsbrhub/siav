...
if($_REQUEST['idCampanha']){
    ...
    if(!$listaEmpresaCampanha){
        (!$infoCampanha->totalEmpresas)? $totalEmpresas = 0: $totalEmpresas = $infoCampanha->totalEmpresas;
    }else{
    ...
    switch ($situacao){
        case "1": //pré-ativa
            ($totalEmpresas == 0) ? $enviar = '' : $enviar = '<button id="btnEnviarAlerta"  type="submit" onclick="exibirEnvioAlerta();"  class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-bell"></span> &nbsp;&nbsp;Enviar Alerta</button>';
            $incluir = '<button type="submit" class="btn btn-primary btn-sm" onclick="incluirEmpresa('.$idCampanha.');"><span class="glyphicon glyphicon-plus"></span> &nbsp;&nbsp;Incluir Empresa</button>';
            break;
        case "2": //ativa
            $enviar = '<button id="btnEnviarAlerta"  type="submit" onclick="exibirEnvioAlerta();"  class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-bell"></span> &nbsp;&nbsp;Enviar Alerta</button>';
            $incluir = '<button type="submit" class="btn btn-primary btn-sm" onclick="incluirEmpresa('.$idCampanha.');"><span class="glyphicon glyphicon-plus"></span> &nbsp;&nbsp;Incluir Empresa</button>';
            break;
        case "3": //inativa
            $enviar = '';
    ...
    $rascunho = $oControle->getRascunhoRecente($_REQUEST['idCampanha']);
...
?>
        <div class="bs-callout bs-callout-primary bg-grey font-12 grey">
            <h4 style="font-size: 14px"><strong>Editar Campanha</strong></h4><br />
            ...
            <form role="form" onsubmit="return false;" id="formEditCampanha">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <h6>Total de Empresas incluídas na Campanha: <a onclick="visualizarEmpresasEdit
                                        (<?=$idCampanha?>,1);" ><span id="totEmpresas" class="label label-primary font-13" style="cursor: pointer">
                                        <?=($totalEmpresas != '')?$totalEmpresas:'0'?></span></a></h6>
                ...
                <div class="row">
                    <div class="col-md-2" >
                        <div class="form-group">
                            ...
                            <select type="text" class="form-control input-sm" id="situacaoAtual" disabled>
                                <option value="1" <?=($situacao == '1')?'selected':''?> >Pré-Ativa</option>
                                <option value="2" <?=($situacao == '2')?'selected':''?>>Ativa</option>
                                <option value="3" <?=($situacao == '3')?'selected':''?>>Inativa</option>
                                <option value="4" <?=($situacao == '4')?'selected':''?>>Encerrada</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2" id="div-inativar">
                        <div class="form-group">
                        <label for="situacao">Inativar:</label>
                        <select type="text" class="form-control input-sm" id="situacao" name="campanha[situacao]">
                            <option value="">Selecione</option>
                            <option value="3">Sim</option>
                            <option value="<?=$situacao?>" selected>Não</option>
                        </select>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <button id="btnEditarCampanha" type="submit" onclick="alterarCampanha();return false" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-ok"></span> &nbsp;&nbsp;Salvar</button>
                                </div>
                            </div>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body bg-grey">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <div class="checkbox checkbox-primary">
                                                        <input type="checkbox" class="styled"  id="todas" name="todas" value="1" onchange="adicionarTodasEmpresas()" <?=$marcarTodas?>> <label class="font-12 grey">Incluir Todas</label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                            ...
                            <div class="no-display" id="carregandoEmpresas">
                                <img src="img/blocksLoading.gif">
                            </div>
                                <div id="divlistaEmpresas" class="bg-grey p-10 content-table <?=
                                (!$listaEmpresaCampanha)?'no-display':''?>">
                                    <form method="post" id="tabelaEmpCampanha" onsubmit="return false;">
                                        ...
                                        ?>
                                        <table class="table table-striped" id="listaEmpresas">
                                            <thead>
                                            ...
                                            if($listaEmpresaCampanha){
                                                foreach ($listaEmpresaCampanha as $emp){
                                                    ...
                                                    ?>
                                                    <tr class="font-12 busca" id="tr-id<?=$emp->idEmpresaCampanha?>" >
                                                        ...
                                                        <td>
                                                            ...
                                                            <span id="EMP<?=$empresa->idEmpresa?>" name="EMP<?=$empresa->idEmpresa?>" ></span>
...
<!--                                        <input type="hidden" id="idCampanha" name="idCampanha" value="--><?//=$_REQUEST['idCampanha']?><!--" />-->
                                        <button id="concluir" name="concluir" class="btn btn-primary font-12" onclick="concluirEmpresa()" >Concluir</button>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control input-sm" id="tipoSelecao" name="alerta[tipoSelecao]">
                                                <option value="todas" <?=($tipoSelecao == 'todas')?'selected':''?>>Todas</option>
                                                <option value="1" <?=($tipoSelecao == '1')?'selected':''?>>Empresas - Cadastros Não Iniciados</option>
                                                <option value="2" <?=($tipoSelecao == '2')?'selected':''?>>Empresas - Iniciadas e não concluídas</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
