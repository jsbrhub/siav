...
class Template {
     ...
     * @param     string $line				contains one line of the content file
     */
    private function identifyVars(&$line, $search = false) {
    ...
    private function createBlocks(&$blocks) {
        ...
        foreach ($blocks as $parent => $block) {
            foreach ($block as $chield) {
                ...
                $this->blocksStruct[$chield] = $this->identifyVars($this->values["{" . $chield . "}"], true);
            }
        }
    }
    ...
    public function clearBlock($block) {
        ...
        $variables = $this->blocksStruct[$block];
